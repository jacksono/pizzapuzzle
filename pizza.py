...
def one_slice(piza, size): #function to cut out one slice of any size without holes starting from col 1 but any row
    start = [0,2] # i have used zero based coords so that the frst line is 0,0 not 1,1
    one_slice = []

    if size[1] > len(piza[start[1]:]) or size[0] > len(piza[0]): #check if slice size can fit in pizza
